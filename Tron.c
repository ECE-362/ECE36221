...
bool detectCollision(Player *p) {
    if((p->x >= WIDTH) || (p->y >= HEIGHT)) {
        ...
        return (1<<(p->x%32)) == (grid[p->y][p->x/32] & 1<<(p->x%32));
    }
}

void drawPath(Player *p) {
    LCD_DrawPoint(2*p->x, 2*p->y, p->color);
    LCD_DrawPoint(2*p->x+1, 2*p->y, p->color);
    LCD_DrawPoint(2*p->x, 2*p->y+1, p->color);
    LCD_DrawPoint(2*p->x+1, 2*p->y+1, p->color);
    grid[p->y][p->x/32] |= 1<<(p->x%32);
}
...
void victoryScreen(bool p1Collision, bool p2Collision) {
    stopTimer();
    gameRunning = false;
    gameReady = false;
    if (p1Collision && p2Collision) {
        LCD_DrawString(60, 100, WHITE, BLACK, "TIE!", 16, 0);
    } else if (p1Collision) {
        LCD_DrawString(60, 100, P2COLOR, BLACK, "Player 2 Wins!", 16, 0);
    } else if (p2Collision) {
        LCD_DrawString(60, 100, P1COLOR, BLACK, "Player 1 Wins!", 16, 0);
    }
    wait(3);
    titleScreen();
}
...
void TIM7_IRQHandler(void) {
    ...
    updatePosition(&p1);
    updatePosition(&p2);
    bool p1Collision = detectCollision(&p1);
    bool p2Collision = detectCollision(&p2);
    if(p1Collision || p2Collision) {
        victoryScreen(p1Collision, p2Collision);
    }
    drawPath(&p1);
    drawPath(&p2);
}
